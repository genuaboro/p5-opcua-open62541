...
if ($async) {
    ...
    is_deeply([$client->{client}->getState()], [SECURECHANNELSTATE_OPEN,
	SESSIONSTATE_ACTIVATED, STATUSCODE_GOOD], "client state");
}
...
$client->iterate(sub {
	...
	return ($client->{client}->getState())[1] == SESSIONSTATE_ACTIVATED;
    }
...
if ($async) {
    ...
    is(($client->{client}->getState())[1], SESSIONSTATE_CLOSED,
	"client bad connection");
}
